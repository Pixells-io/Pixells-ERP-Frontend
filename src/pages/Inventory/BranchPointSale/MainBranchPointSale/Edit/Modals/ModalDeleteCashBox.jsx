...
function ModalDeleteCashBox({ store_id, pos_id, cashBox_name }) {
  ...
  return (
    <Dialog open={modal} onOpenChange={setModal}>
      ...
      <DialogContent className="flex max-w-[400px] flex-col gap-4 border-0 bg-[#242424]">
        ...
        >
          <input type="hidden" hidden name="pos_id" value={pos_id} readOnly />
          <input
            type="hidden"
            hidden
            name="type_option"
            value={"destroyCashBoxBranchTab"}
            readOnly
          />
          <span className="my-4 font-roboto text-xs font-light text-grisDisabled">
            You are trying to delete the cashBox "{cashBox_name}", are you sure?
          </span>
          <DialogFooter>
...
export default ModalDeleteCashBox;
