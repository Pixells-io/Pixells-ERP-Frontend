...
function ModalCancelQuote({ id, name, modal, setModal }) {
  ...
  return (
    <Dialog open={modal} onOpenChange={setModal}>
      <DialogContent className="flex max-w-[400px] flex-col gap-4 border-0 bg-[#242424]">
        ...
        >
          <input type="hidden" hidden name="quote_id" value={id} readOnly />
          <input
            type="hidden"
            hidden
            name="type_option"
          ...
          <span className="my-4 font-roboto text-xs font-light text-grisDisabled">
            Estas tratando de eliminar esta cotizacion "{name}", estas seguro?
          </span>
          <DialogFooter>
